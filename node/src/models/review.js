...
async function getReview(roomId) {
    return new Promise((resolve) => {
        db((err, connection) => {
            const sql =
                'SELECT room.room_name, room.country, room_image.image_1 AS image FROM `room` INNER JOIN `room_image` ON room.room_image_id = room_image.id WHERE room.id = ?';
